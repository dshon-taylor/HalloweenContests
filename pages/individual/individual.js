...
getVotingLinks().then(links => {
  const votingLink = links.find(link => link["Link Name"] === 'Individual Voting URL');
...
Promise.all([
  ...
  // Process winners data for the Individual Costume Contest
  if (!winners || winners.length === 0) {
    console.error("No contest winners data available.");
    return;
  }
  const contestWinners = winners.find(w => w['Contest Name'] === 'Individual Costume Contest');
  if (!contestWinners) {
    console.error("No contest winners for Individual Costume Contest found.");
    return;
  }
  ...
  entriesData.forEach(entry => {
    if (entry[CONTEST_KEY] === 'Individual Costume Contest') {
  ...
  function findAndPlaceEntry(entryToFind) {
    ...
    if (foundEntry) {
      ...
      const card = createEntryCard(foundEntry, true); // Pass true to indicate this is a winner
