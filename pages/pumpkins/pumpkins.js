...
getVotingLinks().then(links => {
  const votingLink = links.find(link => link["Link Name"] === 'Pumpkin Voting URL');
...
Promise.all([
  ...
  // Process winners data for the Pumpkin Contest
  if (!winners || winners.length === 0) {
    console.error("No contest winners data available.");
    return;
  }
  const contestWinners = winners.find(w => w['Contest Name'] === 'Pumpkin Contest');
  if (!contestWinners) {
    console.error("No contest winners for Pumpkin Contest found.");
    return;
  }
  ...
  entriesData.forEach(entry => {
    if (entry[CONTEST_KEY] === 'Pumpkin Contest') {
  ...
  function attachBadges() {
    ...
    if (!winnerEntries || winnerCards.length === 0) {
      console.error("No winner entries found to attach badges.");
      return;
    }
    ...
    if (winnerCards.length >= 3) {
      ...
      firstPlaceBadge.textContent = 'Winner: Best Carved';
      winnerCards[0].querySelector('.entry-details').prepend(firstPlaceBadge);
      
      const secondPlaceBadge = document.createElement('div');
      secondPlaceBadge.classList.add('entry-badge', 'gold');
      secondPlaceBadge.textContent = 'Winner: Best Painted';
      winnerCards[1].querySelector('.entry-details').prepend(secondPlaceBadge);
      
      const thirdPlaceBadge = document.createElement('div');
      thirdPlaceBadge.classList.add('entry-badge', 'gold');
      thirdPlaceBadge.textContent = 'Winner: Most Creative';
